...
def compute_portfolio_noi(isbs_raw, inv_disp, frequency="Quarterly",
    ...
    for vc in parent_vcodes:
        for source_data, target in [
            ...
            cum = compute_cumulative_noi(deal_src, dates, rev_accounts, exp_accounts)
            periodic = cumulative_to_periodic(cum, dates)
            for dt, val in periodic.items():
                target[dt] = target.get(dt, 0) + val

    actual_agg = aggregate_periodic(actual_periodic_total, frequency)
    uw_agg = aggregate_periodic(uw_periodic_total, frequency)
