...
- **Anticipated_Exit** (Underwritten Exit): From MRI `event_dates` table (`veventtype = 'Asset Management' AND vevent = 'U/W Exit'`). Mapped via `Prop_Info_Core.sql` as `Anticipated_Exit` column.
...
- **General Information** — Left column: Partner, Location, Asset Type / Year Built (combined with `|` separator), # Units / SF. Right column: Investment Strategy, Date Closed, Underwritten Exit, Current Anticipated Exit (from `event_dates` table, `vEventType='Disposition', vEvent='Closing', vDateType='Projected'`).
...
- Underwritten PE (Projected IS): `UW_PE_DIST_ACCT` 7071 (PE distributions — ROE numerator), `UW_PE_ROC_ACCT` 7073 (PE return of capital — reduces weighted avg capital denominator, NOT counted as distributions). 7071 is YTD cumulative converted to periodic by `_get_uw_pe_periodic()`. 7073 uses `_get_uw_roc_events()` which detects distinct ROC events from cumulative jumps (planned ROC appears on event date then repeats flat through year-end; only the jump is an event). Capped so capital never goes negative.
