...
The rule is REJECT WHAT CANNOT BE TRUE, WARN WHAT IS MERELY ODD:
  ...
  refused   not a date; earlier than the period end (you cannot close a
            period before it has ended)
...
with app.app_context():
    ...
    check("a deadline before the period end", msg is not None, msg or "")
    check("  and the message says why", bool(msg) and period_end in msg, msg or "")
    ...
    for anchor in dated:
        ...
        if day_before.isoformat() < period_end:
            continue          # no room between the period end and the anchor
    ...
    stale = [f"{s_['label']}={s_['due_date']}" for s_ in ws.cycle_steps(cid)
             if s_.get("due_date") and str(s_["due_date"])[:10] < period_end]
    if stale:
