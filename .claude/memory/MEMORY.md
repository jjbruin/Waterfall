...
- [treasury.md](treasury.md) — **TREASURY, the bank side of the close** (built Sep 17-18 2026, live `v507`): PNC activity and statement import, the three-way tie, the matcher and its reconciling items, and why `current_available` is deliberately blank. **Production holds ZERO statements — the June load has not been done** (open_items §7.10), so the seeder correctly refuses; a statement whose account is not registered is now HELD and asked about rather than dropped, its PDF is kept, and all six `tr_*` tables are protected. Also the ACCESS MODEL for the whole accounting section — `roles_exactly` vs `role_required`, and the six writes that had no gate at all. Read before touching `/treasury` or any accounting permission.
