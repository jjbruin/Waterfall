...
- [treasury.md](treasury.md) — **TREASURY, the bank side of the close** (built Sep 17-18 2026, live `v508`): PNC activity and statement import, the three-way tie, the matcher and its reconciling items, and why `current_available` is deliberately blank. **Production holds ZERO statements — the June load has not been done** (open_items §7.10), so the seeder correctly refuses; a statement whose account is not registered is now HELD and asked about rather than dropped, its PDF is kept, and all six `tr_*` tables are protected. Also the ACCESS MODEL for the whole accounting section — `roles_exactly` vs `role_required`, and the six writes that had no gate at all. Read before touching `/treasury` or any accounting permission.
...
- [session_handoff.md](session_handoff.md) — **CURRENT rolling handoff (Sep 23 2026, `v523` live). READ THIS FIRST.** Top section: the Sep 22-23 BUDGET IMPORT work — it **had never once succeeded on production**, because `commit()` quoted `"vcode"` while the table carries `vCode` (case-sensitive on PostgreSQL, insensitive on SQLite), so eight spreadsheet versions from Jack were all doomed at the last step whatever their layout. Also: the parser read only column A; a sheet with two tables side by side read names from one and figures from the other; the account now decides the category and the dropdown is gone; and many lines may share one account. Its method note: **a bundle check that greps a 551-byte SPA shell proves nothing** — index.html names only the entry chunk. Below that: the Sep 20-21 VALIDATION SCREEN work — top section: the Sep 20–21 VALIDATION SCREEN work — settling a mismatch with a required reason and a checked document citation, the change report, a fixed CAM charge finally validated (dated from calendar years, lease years against rent commencement, or a verbatim period), and a document added to a scanned tenant re-reading that tenant's whole set. Its three lessons: run a re-extraction WITH A DIFF (it found a pro-rata estimate being compared as a cap), do the targeted run before the full one, and inject the defect — four of my own checks were weaker than they looked. Below that: the Sep 20 LEASE CORPUS work — the document classifier reading the folder instead of the file, the gate that had to widen with it, the ordering regression a before/after diff caught, and scanned PDFs now read as images with no OCR stack. Below that: ONE NUMBER ONE ENGINE (Jim's standing rule and the $633,807 pref split that produced it), the lease rent-in-force resolution, and the CFO's GL/IA query tool, and statement drilldown on the workbench. Above it, Sep 19: the held-statement flow and why treasury is still empty on production. Below that a STILL OPEN list — four decisions waiting on Jim (the capital-balance floor on 15 deals, the MRI password in git, GL/IA access, the IA date bound) and five things needing production data. Older strata beneath: Sep 17-18 treasury, Sep 15 workpapers, Sep 11 valuations, Sep 10 ownership.
