...
- [session_handoff.md](session_handoff.md) — **CURRENT rolling handoff (Sep 25 2026, `v526` live). READ THIS FIRST.** Top section: AM's second valuation list — the NOI tie-out that used its own NOI definition (`v525`), critical-only checks, and in `v526` the Valuation/UW column toggle, UW debt service in the Budget column, Estimate line overrides and budgeted occupancy. Carry forward: **UW debt service is ONE figure, 7010** (never split it); on production only 78 of 131 UW-covered records carry it; **7030 is "Replacement Reserve Deposit" in the COA yet treated as interest**; and **v524 failed on boot because SQLAlchemy 2.1 shipped Sep 24** (now pinned `<2.1`) — any rebuild would have failed. Below that: Sep 22-23 BUDGET IMPORT (it had never once succeeded on production — `"vcode"` vs `vCode`), Sep 20-21 validation screen, Sep 20 lease corpus, one number one engine, GL/IA query, treasury, workpapers, valuations, ownership.
